...
namespace ValhallaVaultCyberAwareness.Cache
{
	public class ByIdCachePolicy : IOutputCachePolicy
	{
		...
		{
			//Extract all id's we are interested in to use for setting/invalidating the cache.
			...
			if (categoryId != null)
			{
				context.Tags.Add("category_" + categoryId.ToString()!);
			}
			if (segmentId != null)
			{
				context.Tags.Add("segment_" + segmentId.ToString()!);
			}
			if (subCategoryId != null)
			{
				context.Tags.Add("subCategory_" + subCategoryId.ToString()!);
			}
			if (questionId != null)
			{
				context.Tags.Add("question_" + questionId.ToString()!);
			}
